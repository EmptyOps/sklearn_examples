...
def main_process(t_no):    

    from keras import backend as K
    K.clear_session()

    input_to_be_predicted = np.array( json.load( open( sys.argv[3].replace("tcls", str(t_no)) ) ) ) 
    input_to_be_predicted_labels = np.array( json.load( open( sys.argv[14].replace("tcls", str(t_no)) ) ) ) if len(sys.argv) >= 15 else None

    outfile_path = sys.argv[15].replace("tcls", str(t_no)) if len(sys.argv) >= 16 else None
    outfile_path_prob = sys.argv[19].replace("tcls", str(t_no)) if len(sys.argv) >= 20 else None

    model = None
    import os.path

    is_evaluation_mode = False
    print( 'thread no ' + str(t_no) )
...
if is_python_mutli_threading_inside_process == 1:

    import threading 

    """
    total_threads = int(sys.argv[24]) if len(sys.argv) >= 25 else 0
    ts = {}
    for t_no in range(1, total_threads):
        
        # creating thread 
        ts[t_no] = threading.Thread(target=main_process, args=(t_no,)) 
    ...
    """
    # creating thread 
    ts1 = threading.Thread(target=main_process, args=(1,)) 
    t2 = threading.Thread(target=main_process, args=(2,)) 
